...
public interface WBService {
    ...
    Call<List<FriendTimeLine>> getFriendsTimeLine(@Query("access_token") String accessToken // 采用OAuth授权方式为必填参数，OAuth授权后获得。
...
//                                                        @Query("count") int count, // 单页返回的记录条数，最大不超过100，默认为20。
//                                                        @Query("page") int page, // 返回结果的页码，默认为1。
//                                                        @Query("base_app") int baseApp, // 是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
//                                                        @Query("feature") int feature, // 过滤类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0。
//                                                        @Query("trim_user") int trimUser // 返回值中user字段开关，0：返回完整user字段、1：user字段仅返回user_id，默认为0。
    );
}
